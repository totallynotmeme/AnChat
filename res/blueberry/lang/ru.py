...
1. Скачать 2 файла с репозитории проекта:
  - hosting/server-http.py
  - hosting/forward-localhostdotrun.py
2. Открыть 2-ой файл в своём редакторе кода и поиграться с настройками
...
Важно: этот сервер продержится только небольшое время, около 5-10 минут.\
 ...
 радмин". Тогда тебе нужно будет скачать только один скрипт-сервер: server-socket.py
