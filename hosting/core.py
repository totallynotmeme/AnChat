# /hosting/core.py
# module which contains protocol.tobits and wordip functions from core script


# /res/connection.py
def socket_tobits(addr, port):
    if addr.count(".") != 3:
...
def wordip_encode(bitarray):
    bitarray += bin(random.randint(0, 256))[2:].zfill(8)
    ...
    for _ in range(11):
        ...
        for i in bitarray:
            i = int(i)
            new_bitarray += str(i ^ last_bit)
            last_bit = i
        bitarray = new_bitarray
    
    # convert bitarray to int (and then base 2309)
    number = int("1" + bitarray, 2)
    words = []
    while number > 0:
        number, rem = divmod(number, len(wordbase))
        words.append(wordbase[rem])
    
    return " ".join(words)


def wordip_decode(words):
    # strip the //passphrases
    if "//" in words:
        words = words.split("//", 1)[0]
    words = words.split()
    
    # converting from words (base 2309)
    number = 0
    base = len(wordbase)
    for exp, i in enumerate(words):
        number += wordbase.index(i) * base ** exp
    
    # converting int to bitarray
    bitarray = bin(number)[3:]
    ...
    for _ in range(11):
        ...
        for i in bitarray:
            if i == "1":
                last_bit = flipper[last_bit]
            new_bitarray += last_bit
        bitarray = new_bitarray
